...
def perceptron(data_train, data_test, _fout, _ftime):
	...
	for _train_samples in range(1000, 21000, 1000):
		...
		print("Creating Perceptron...")
        	# Cria um perceptron
		perceptron = Perceptron(tol=1e-5, max_iter=100000)

	        print ('Fitting Perceptron...')
		perceptron.fit(X_train[0:_train_samples], y_train[0:_train_samples])
		...
		_ftime.write("perceptron_" + str(_train_samples) + ". Start: " + str(start) + ". Stop: " + str(stop) +". Delta: " + str(delta) )
		_ftime.write("\n")
		...
		plt.figure()
		plot_confusion_matrix(cm, classes=class_names, normalize=True, title="Perceptron: "+str(_train_samples))
		plt.savefig(ml_lab1_path + "/perceptron/perceptron_"+ str(_train_samples)+ "_cm.png")
...
def main(args):
	...
	try:
		...
		fout = open(ml_lab1_path + "/perceptron/results_perceptron", "w")
		#Abre arquivo para histórico das datas
		ftime = open(ml_lab1_path + "/perceptron/results_time_perceptron", "w")

		perceptron(ml_lab1_path + "/data/train.txt", ml_lab1_path + "/data/test.txt", fout, ftime)
