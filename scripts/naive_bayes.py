...
def naive_bayes(data_train, data_test, _fout, _ftime):
	...
	for _train_samples in range(1000, 21000, 1000):
		...
		naive = gnb.fit(X_train[0:_train_samples], y_train[0:_train_samples])
		...
		#Delta de tempo
		#delta_start = datetime.timedelta(start)
		#delta_stop = datetime.timedelta(stop)
		delta = stop - start

		_ftime.write("nb_" + str(_train_samples) + ". Start: " + str(start) + ". Stop: " + str(stop) +". Delta: " + str(delta) )
		_ftime.write("\n")
		...
		plt.figure()
		plot_confusion_matrix(cm, classes=class_names, normalize=True, title="Naive Bayes: "+str(_train_samples))
		plt.savefig(ml_lab1_path + "/naive_bayes/nb_"+ str(_train_samples)+ "_cm.png")
...
def main(args):
	...
	try:
	        rospy.init_node('n_lab_2_naive', anonymous=True)
		...
		fout = open(ml_lab1_path + "/naive_bayes/results_naive_bayes", "w")
		#Abre arquivo para histórico das datas
		ftime = open(ml_lab1_path + "/naive_bayes/results_time_naive_bayes", "w")

		naive_bayes(ml_lab1_path + "/data/train.txt", ml_lab1_path + "/data/test.txt", fout, ftime)
