...
def knn(data_train, data_test, _fout, _ftime):
	...
	for _train_samples in range(1000, 21000, 1000):
		...
		neigh = KNeighborsClassifier(n_neighbors=3)
	        ...
	        print ('Fitting KNN...')
		neigh.fit(X_train[0:_train_samples], y_train[0:_train_samples])
		...
		_ftime.write("knn_" + str(_train_samples) + ". Start: " + str(start) + ". Stop: " + str(stop) +". Delta: " + str(delta) )
		_ftime.write("\n")
		...
		plt.figure()
		plot_confusion_matrix(cm, classes=class_names, normalize=True, title="K Nearest Neighbors: "+str(_train_samples))
		plt.savefig(ml_lab1_path + "/knn/knn_"+ str(_train_samples)+ "_cm.png")
...
def main(args):
	...
	try:
		...
		fout = open(ml_lab1_path + "/knn/results_knn", "w")
		#Abre arquivo para histórico das datas
		ftime = open(ml_lab1_path + "/knn/results_time_knn", "w")

		knn(ml_lab1_path + "/data/train.txt", ml_lab1_path + "/data/test.txt", fout, ftime)
